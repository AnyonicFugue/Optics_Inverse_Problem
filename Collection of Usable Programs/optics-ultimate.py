...
def iterate_gpu(di, input_spectrum):
    ...
    np.setbufsize(min(lambda_num*n, 2048*4096))
...
def set_sine(l, amplitude):
    ...
    l_list = np.linspace(1000/lambda_min, 1000/lambda_max,
                         lambda_num, dtype=np.float32)

    for i in range(lambda_num):
        lambda1 = 1000/l_list[i]
...
def main():
    ...
    #goal_spectrum = set_sine(3, 0.3)
    #goal_spectrum = set_sawtooth(5, 0.4)
    add_peak(500, 10,goal_spectrum)
