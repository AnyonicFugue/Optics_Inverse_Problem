...
D = np.array([[[1, 1], [N(i), -N(i)]] for i in range(n)], dtype=np.complex128)
D_inv = np.array([[[0.5, 0.5/N(i)], [0.5, -0.5/N(i)]]
                 for i in range(n)], dtype=np.complex128)
...
def set_T_new(T_dev, di_host):
    ...
    set_T[(n,blocknum), blocksize](di_dev, T_dev, lambda_list_dev,D_dev,D_inv_dev)
...
def E_pre_new(T_dev, di_host, spec_out):  # both on host
    #T_dev = cuda.device_array((lambda_num, n, 2, 2), dtype=np.complex128)
...
def get_J_gpu(spectrum_out, di, J, left_T, right_T, l_list,D_dev,D_inv_dev):
    ...
    tmp=cuda.local.array((2,2),dtype=np.complex128)
    fullT=cuda.local.array((2,2),dtype=np.complex128)
...
def iterate_gpu(times, di, input_spectrum):
    ...
    JTJ = np.empty((n, n))
    #P = np.zeros((lambda_num, n, 2, 2), dtype=np.complex128)
    T = np.zeros((lambda_num, n, 2, 2), dtype=np.complex128)  # T=D*P*inverse(D)
    left_T = np.zeros((lambda_num, n, 2, 2), dtype=np.complex128)
    right_T = np.zeros((lambda_num, n, 2, 2), dtype=np.complex128)
    output = np.zeros(lambda_num)
    residual = np.empty(lambda_num)

    J_dev = cuda.device_array((lambda_num, n))
    #JTJ_dev = cuda.device_array((n, n))
    #P_dev = cuda.to_device(np.array((lambda_num, n, 2, 2), dtype=np.complex128))
    T_dev = cuda.device_array((lambda_num, n, 2, 2), dtype=np.complex128)
    left_T_dev = cuda.device_array((lambda_num, n, 2, 2), dtype=np.complex128)
    right_T_dev = cuda.device_array((lambda_num, n, 2, 2), dtype=np.complex128)

    residual_dev = cuda.device_array(lambda_num)
    output_dev = cuda.to_device(output)
    di_dev = cuda.to_device(di)

    T1 = np.zeros((lambda_num, n, 2, 2), dtype=np.complex128)
    T2 = np.zeros((lambda_num, n, 2, 2), dtype=np.complex128)
