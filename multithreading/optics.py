...
thread_num = 12
thread_segment_size = math.ceil(lambda_num/thread_num)
...
D_inv = np.array([np.mat([[0.5, 0.5/N(i)], [0.5, -0.5/N(i)]])
                 for i in range(n)])


def P(i, lambda1, di):
    # 定义矩阵P
    return np.mat([[cmath.exp(-(0+1j)*2*math.pi*di[i]/lambda1), 0], [0, cmath.exp((0+1j)*2*math.pi/lambda1*di[i])]])
...
def get_J(J, thread_index, di):  # 第一个参数是可修改的雅可比阵；第二个参数标示线程序号
    ...
    # 对每个波长分别计算
    for j in range(thread_index*thread_segment_size, (thread_index+1)*thread_segment_size):
        # 如果超出波长范围则返回
        if j >= lambda_num:
            return
...
def iterate(times, di, input_spectrum):  # 最速下降迭代函数；di为厚度初值
    ...
    thread_list = [0 for k in range(thread_num)]

    for i in range(0, times):
        start = time()
        for k in range(thread_num):
            thread_list[k] = threading.Thread(target=get_J, args=(J, k, di))
            thread_list[k].start()
        for k in range(thread_num):
            # if(thread_list[k].is_alive()):
            thread_list[k].join()

        print("Get Jacobian time:"+str(time()-start))
        ...
        movement1 = np.linalg.inv(
            JTJ+damp*np.matlib.eye(n))*(J.T)*(np.mat(residual).T)
        movement2 = np.linalg.inv(
            JTJ+(damp/v)*np.matlib.eye(n))*(J.T)*(np.mat(residual).T)
        ...
        print("Get improvement time:"+str(time()-start))
...
initial_d = np.array([random.uniform(d-delta_d, d+delta_d)
                     for i in range(1, n+1)])
s_in = np.array([0.3*(np.cos(a/50))**2 for a in lambda_list])
