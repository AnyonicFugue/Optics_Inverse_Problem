...
def test():
    ...
    di=np.array([random.uniform(d-delta_d,d+delta_d) for i in range(1,n+1)])
    istest=False

    for j in range(0,lambda_num):
        spectrum_out[j]=E_pre(lambda_min+lambda_step*j)
        test_spectrum[j]=spectrum_out[j]#测试光谱
        residual[j]=input_spectrum(lambda_min+lambda_step*j)-spectrum_out[j]
    

    test_d=np.array([j for j in di])#测试厚度

    di=np.array([random.uniform(d-delta_d,d+delta_d) for i in range(1,n+1)])#重新随机厚度

    iter_num=int(input("iternum:"))
    iterate(iter_num)
